...
- `--fresh-cache` - runs the `pnpm update` step with a temporary empty pnpm cache directory.

> `--sample` used together with `--ckeditor-only` updates only `peerDependencies` in selected samples.

> **ProTip**: This script (with `--ckeditor-only --fresh-cache` flags) is run by [release script](#release), so it does not have to be run separately in normal circumstances.
