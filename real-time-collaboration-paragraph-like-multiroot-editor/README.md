...
Because `MultiRootEditor` does not inject its toolbar into the page automatically, the application manually appends `editor.ui.view.toolbar.element` (and `editor.ui.view.menuBarView.element`) to a dedicated container element after the editor is created.
